...
export function registerEVMTools(server: McpServer) {
      ...
      try {
        ...
        return {
          content: [{
            ...
            text: JSON.stringify(
              {
                  "CoreStake": "0xf793A4faD64241c7273b9329FE39e433c2D45d71",
                  "CoreVestV2": "0xaEf0a72A661B82CB1d871FCA5117486C664EeF13",
                  "ValidatorRegistry": "0x7A7dB2f4d073B59f9Af4beF32bc16C804610Edc9",
                  "VERSE": "0x72D2bFb14b3351d17A63Cd4c8085E034e313c54c",
                  "VIBE": "0x8e7C7f0DF435Be6773641f8cf62C590d7Dde5a8a",
                  "VTROStake": "0x27436fccC5b1B3Ff50Bd74c413a9DCfEd4bA2d0b",
                  "VTRU": "0x931AC5400ba4BD2d882Ef6Bc3C5714113f2a4d29",
                  "VUSD": "0x1D607d8c617A09c638309bE2Ceb9b4afF42236dA",
                  "wVTRU": "0x3ccc3F22462cAe34766820894D04a40381201ef9",
                  "BoosterPerks": "0x8A66d12De0276Eb6eF9dd493038531b00E9B9B4b"
              }
              , null, 2)
          }]
        };
        ...
        return {
          content: [{
            type: "text",
            text: `Error fetching core contracts: ${error instanceof Error ? error.message : String(error)}`
          }],
          isError: true
        };
      }
  ...
  //     network: z.string().optional().describe("Network name (e.g., 'vitruveo', 'optimism', 'arbitrum', 'base', etc.) or chain ID. ENS resolution works best on Ethereum mainnet. Defaults to Ethereum mainnet.")
  //   },
  //   async ({ ensName }) => {
  //     try {
  ...
  //       const address = await services.resolveAddress(ensName);
  ...
  server.tool(
    ...
    async ({ blockNumber }) => {
      try {
        const block = await services.getBlockByNumber(blockNumber);
  ...
  server.tool(
    ...
    async ({ address }) => {
      try {
        const balance = await services.getVTRUBalance(address);
  ...
  server.tool(
    ...
    async ({ txHash }) => {
      try {
        const receipt = await services.getTransactionReceipt(txHash as Hash);
  ...
  server.tool(
    ...
    async ({ to, value, data }) => {
      try {
        ...
        const gas = await services.estimateGas(params);
  ...
  //     amount: z.string().describe("Amount to send in VTRU as a string (e.g., '0.1')"),
  //     network: z.string().optional().describe("Network name (e.g., 'vitruveo', 'optimism', 'arbitrum', 'base', etc.) or chain ID. Supports all EVM-compatible networks. Defaults to Ethereum mainnet.")
  //   },
  //   async ({ privateKey, to, amount }) => {
  //     try {
  //       const txHash = await services.transferVTRU(privateKey, to, amount);
  ...
  //     network: z.string().optional().describe("Network name (e.g., 'vitruveo', 'optimism', 'arbitrum', 'base', etc.) or chain ID. Supports all EVM-compatible networks. Defaults to Ethereum mainnet.")
  //   },
  //   async ({ privateKey, tokenAddress, toAddress, amount }) => {
  //     try {
  ...
  //     network: z.string().optional().describe("Network name (e.g., 'vitruveo', 'optimism', 'arbitrum', 'base', 'polygon') or chain ID. Defaults to Ethereum mainnet.")
  //   },
  //   async ({ privateKey, tokenAddress, spenderAddress, amount }) => {
  //     try {
  //       // Get the formattedKey with 0x prefix
  //       const formattedKey = privateKey.startsWith('0x') 
  //         ? privateKey as `0x${string}` 
  ...
  //   },
  //   async ({ privateKey, tokenAddress, tokenId, toAddress }) => {
  //     try {
  //       // Get the formattedKey with 0x prefix
  //       const formattedKey = privateKey.startsWith('0x') 
  //         ? privateKey as `0x${string}` 
  ...
  //   },
  //   async ({ privateKey, tokenAddress, tokenId, amount, toAddress }) => {
  //     try {
  //       // Get the formattedKey with 0x prefix
  //       const formattedKey = privateKey.startsWith('0x') 
  //         ? privateKey as `0x${string}` 
  ...
  server.tool(
    ...
    async ({ contractAddress, abi, functionName, args = [] }) => {
      try {
        ...
        const result = await services.readContract(params);
  ...
  //     network: z.string().optional().describe("Network name (e.g., 'vitruveo', 'optimism', 'arbitrum', 'base', 'polygon') or chain ID. Defaults to Ethereum mainnet.")
  //   },
  //   async ({ contractAddress, abi, functionName, args, privateKey }) => {
  //     try {
  //       // Parse ABI if it's a string
  //       const parsedAbi = typeof abi === 'string' ? JSON.parse(abi) : abi;
  ...
  server.tool(
    ...
    async ({ address }) => {
      try {
        const isContract = await services.isContract(address);
  ...
  server.tool(
    ...
    async ({ tokenAddress }) => {
      try {
        const tokenInfo = await services.getERC20TokenInfo(tokenAddress as Address);
